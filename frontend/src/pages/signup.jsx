...
function GenderDropdown({ value, onChange }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className={` ${value ? "text-white/95" : "text-slate-300"}`}>
          {options.find((o) => o.key === value)?.label || "Select gender"}
        </span>
        <svg
          width="18"
          height="18"
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(4,18,28,0.96), rgba(7,34,44,0.94))",
...
export default function Signup() {
  ...
  function validate() {
    ...
    if (form.parentPassword !== form.parentConfirm)
      return "Parent passwords do not match.";
    return null;
  }
    ...
    try {
      ...
      setErrorMsg(
        err?.response?.data?.message ||
          "Network or server error. Check backend."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden font-lexend text-white antialiased">
      ...
      <div className="pointer-events-none absolute right-0 bottom-0 z-10">
        <svg
          viewBox="0 0 520 520"
          className="w-[44vw] sm:w-[36vw] md:w-[30vw] lg:w-[24vw]"
        >
          <defs>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-slate-200/80 mt-1 max-w-md">
                Parent-assisted signup. Saarthi will cheer every step.
              </p>
            </div>
          ...
          <div className="relative">
            ...
            <form onSubmit={handleSignup} className="relative z-10 space-y-4">
              {/* Child name */}
              <div>
                <label className="text-xs text-slate-200/80 mb-1 block">
                  Child's name
                </label>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={onChange("parentPassword")}
                    type={showParentPass ? "text" : "password"}
                    placeholder="Create a parent password"
                    aria-label="Parent password"
                    className="w-full rounded-xl px-4 py-3 pr-12 bg-white/5 border border-white/6 focus:border-cyan-300 outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setShowParentPass((s) => !s)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-white/90 p-1 rounded"
                    aria-label={
                      showParentPass ? "Hide password" : "Show password"
                    }
                  >
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    value={form.parentConfirm}
                    onChange={onChange("parentConfirm")}
                    type={showParentConfirm ? "text" : "password"}
                    placeholder="Repeat parent password"
                    aria-label="Confirm parent password"
                    className="w-full rounded-xl px-4 py-3 pr-12 bg-white/5 border border-white/6 focus:border-cyan-300 outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setShowParentConfirm((s) => !s)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-white/90 p-1 rounded"
                    aria-label={
                      showParentConfirm
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  required
                  type="checkbox"
                  className="w-4 h-4 rounded border-white/20 bg-white/4"
                />
                <label htmlFor="consent" className="text-sm text-slate-200/80">
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                >
                  ...
                  <svg
                    className="absolute right-3 w-8 h-8"
                    viewBox="0 0 36 36"
                    fill="none"
                  >
                    <circle
              ...
              <p className="text-center mt-3 text-xs text-slate-300">
        ...
        {success && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative z-60 text-center max-w-sm px-6 py-6 rounded-3xl bg-gradient-to-tr from-slate-900/95 to-slate-800/80 border border-white/8 shadow-[0_30px_80px_rgba(2,6,23,0.7)]"
            >
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: [0, 1.08, 1] }}
                transition={{ duration: 0.48 }}
                className="mx-auto w-32 h-32 rounded-full bg-gradient-to-tr from-green-300 to-cyan-200 shadow-inner mb-4 grid place-items-center text-6xl"
              >
                🎉
              </motion.div>
              ...
              <motion.p
                ...
                className="text-sm text-slate-200/85 mb-4"
              >
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {particles.map((p, i) => (
                <motion.div
                  ...
                  animate={{
                    y: 820 + Math.random() * 160,
                    x: `${p.left + (Math.random() - 0.5) * 22}%`,
                    opacity: 1,
                    scale: 1.3,
                    rotate: p.rot + 360,
                  }}
                  transition={{
                    delay: p.delay + i * 0.005,
